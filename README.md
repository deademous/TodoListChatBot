...
6.1. Пользователь:Вариант А: Отправляет текстом время, например 14:30.
6.2. Вариант Б: Нажимает кнопку [ ⏰ Без времени ].
7. Бот: "Готово! Задача создана."Бот присылает "карточку" созданной задачи с кнопками управления.
...
2. Далее бот присылает список "карточек" для всех задач, у которых: Дата = Сегодня, Дата = Без даты и статус = Активна.
...
1. Бот присылает сообщение: "Вечерний обзор. Невыполненные задачи за сегодня:"Далее присылает список "карточек" для всех задач, у которых:( Дата = Сегодня или Дата = Без даты ) и статус = Активна.
2. Сразу после этого списка бот присылает отдельное сообщение:"Задачи на сегодня, которые не были выполнены, автоматически переносятся на завтра (без времени).Хотите спланировать завтрашний день?
